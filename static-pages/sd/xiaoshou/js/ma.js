...
//右侧当前所有工单
$(function(){
	hqnetType=sessionStorage.getItem("hqnetType");
...
function youcel(){
	...
	if(hqnetType==1){
	  //获取下拉框的val值
		...
		var installedArea = $('#installed_area').val();
		var byGrid = $('#byGrid').val();
		var request={
				param:{
					"taskId":nextTaskId,
					"accNbr":"",
        ...
        success: function(data){
	    	...
	    	if(data.error.code==1){
				if(hqnetType==1){
					...
					var workorderStr='<li id="workorder">'+
								'<div style="width: 83px;"><span>操作</span></div>'+
								'<div style="width:84px"><span>宽带/固话账号</span></div>'+
								'<div><span>联系电话</span></div>'+
								'<div><span>用户名</span></div>'+
								'<div><span>开户时间</span></div>'+
								'<div><span>产品名称</span></div>'+
								'<div><span>装机小区</span></div>'+
								'<div><span>对应网格</span></div>'+
								'<div><span>执行人</span></div>'+
								'<div><span>执行状态</span></div>'+
								'<div><span>归属状态</span></div>'+
								'<div><span>竣工情况</span></div>'+
	    		...
	    		$.each(data.data.taskList,function(z,y){
					...
					var operatorStr = '<a href="information.html?hqid='+ y.id+'" target="_blank"><span class="xaing1" style="width: 34px;">详情</span></a><span class="che1" style="width: 34px;">撤销</span>';
					 if(a[3]=='转出'&&a[2]=='未执行'){ //如果是未执行&&是转出的  添加撤销转派的按钮
					 	operatorStr='<a  class="xql" href="information.html?hqid='+ y.id+'" target="_blank"><span class="xaing1" style="width: 34px;">详情</span></a><span class="cxl che2" id="'+y.id+'" onclick="chexiao(this)" style="width: 34px;">撤销</span>';
					 }
					 if(hqnetType==1){
						...
						var str1='<li class="shensdas">'+
								'<div style="width: 83px;"><span>'+operatorStr+'</span></div>'+ 	//操作
								'<div id="staus_'+y.id+'" style="width:84px"><span class="title" title="'+y.accNbr+'">'+y.accNbr+'</span></div>'+ 	//宽带、固话账号
								'<div><span class="title" title="'+ a[10]+'">'+a[10]+'</span></div>'+ //联系电话
								'<div><span class="title" title="'+ a[11]+'">'+a[11]+'</span></div>'+ //用户名
								'<div><span class="title" title="'+ a[12]+'">'+a[12]+'</span></div>'+ //开户时间
								'<div><span class="title" title="'+ a[13]+'">'+a[13]+'</span></div>'+ //产品名称
								'<div><span class="title" title="'+ a[14]+'">'+a[14]+'</span></div>'+//装机小区
								'<div><span class="title" title="'+ a[15]+'">'+a[15]+'</span></div>'+ //对应网格
								'<div><span class="title" title="'+y.name+'">'+y.name+'</span></div>'+// 执行人
								'<div><span class="title" title="'+a[2]+'">'+a[2]+'</span></div>'+ //执行状态太
								'<div id="zi_'+y.id+'"><span class="title" title="'+a[3]+'">'+a[3]+'</span></div>'+  	// 归属状态
								'<div id="jun_'+y.id+'"><span class="title" title="'+a[4]+'">'+a[4]+'</span></div>'+	//竣工
