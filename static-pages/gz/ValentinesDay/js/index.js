...
function login(){
    var str='<div class="load"></div>'+
        '<div id="login" >'+
        '<div class="header"><span class="title">情人节赠送包</span><span class="span" onclick="loginclose();" title="关闭"><img style="width: 20px;margin-top:30px;margin-right: 11px;" src="images/close.png"/></span></div>'+
        ...
        success: function(data){
           ...
           if(data.status==0){
            ...
            if(data.status==3){
                ...
                $('#successMessage').html('<p class="success">恭喜您！成功为'+phone2+'订购520M省内流量，系统将在24小时内为您进行受理，受理结果将通过短信进行告知。您还获得额外2次抢红包机会，在微信后台回复“红包”即可参与！</p>');
                $("#Tips").html("");
