 ...
 * */
 function daysOrderBtn() {
     ...
     pram["where"]= 1;
     pram["openId"]="";
      $("#daysBtn").html('<div class="days-but" onclick="daysErrorClick()">点击订购</div>');
     $.ajax({
         type: 'POST',
         url: url,
         data: JSON.stringify(pram),
         ...
         success: function(data){
            ...
            $('.days-scucces').html('<p class="days-scu">恭喜您,成功抢购'+flowStr+',系统将在24小时内为您受理,受理结果将通过短信进行告知.感谢您的参与!</p>');
            }else{
                $('.days-errMsg').html(data.message);
