/**
 * Created by mysheng on 2016/7/13.
 */
...
function login(){
    var str='<div class="load"></div>'+
        ...
        '</div><div style="color: red;margin: 10px;text-align: left;" id="Tips"><p >Tips：仅限贵州电信用户办理，非电信用户、欠费</p><p style="text-indent: 3em;">用户和非实名认证用户均不能办理。</p></div>' +
        '</div>';
    $("#loginStr").html(str);
}
function clearError(){
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            if(data.status==0){
                ...
                $('#successMessage').html('<p class="success">对不起！您输入的手机号不能办理！</p>');
            }else{
                buttonTime(btn);
            }
        },
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            if(data.status==0){
                ...
                $('#successMessage').html('<p class="success">恭喜您!成功为'+phone2+'订购520M省内流量,系统将在24小时内为您进行受理.感谢您的参与!</p>');
                $("#Tips").html("");
